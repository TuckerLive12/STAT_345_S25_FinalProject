...
Compilation <- data.frame(CountyID = counties$COUNTYFP[sample[1]], No = 1, State = counties$STATE_NAME[sample[1]], County = counties$NAME[sample[1]])

for(i in 2:length(sample)) {
  Compilation <- rbind(Compilation, list(counties$COUNTYFP[sample[i]], i, counties$STATE_NAME[sample[i]], counties$NAME[sample[i]]))
}
...
Compilation <- filter(Compilation, CountyID < 60)
...
for (i in 1:nrow(Compilation)) {
  ...
  j <- which(counties$COUNTYFP == Compilation$CountyID[i] & counties$NAME == Compilation$County[i] & counties$STATE_NAME == Compilation$State[i])
