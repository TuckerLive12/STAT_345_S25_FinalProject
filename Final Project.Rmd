...
for (i in 1:50) {
  data <- read.csv(Filepaths$file_path[i]) |>
    mutate(COUNTYFP = as.numeric(COUNTYFP)) |>
    mutate(STATE_ID = as.numeric(STATE_ID)) |>
  ...
  for (j in 1:nrow(data)) {
    x <- which(Compilation$COUNTYFP == data$COUNTYFP[j] & Compilation$STATE_ID == data$STATE_ID[j])
    Compilation$MOSTFREQUENT[x] <- data$SUFFIX[j]
    Compilation$N[x] <- data$n[j]
  }
...
Compilation1 <- Compilation |>
  filter(!(MOSTFREQUENT %in% c("Ar", "Cir", "Cr", "Fs", "Gove", "K", "Lake", "Liv", "Lr", "P", "Pl", "Pvt", "R", "Road", "S", "Sfc", "T", "Trl", "Way")))
