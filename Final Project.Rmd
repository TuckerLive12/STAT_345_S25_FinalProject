...
```{r Jack Kucera}
Alabama_Roads_2023_Backup <- read_csv("./Data_Scraping_and_Database/Alabama_Roads_2023_Backup_Backup.csv", col_types = cols(LINEARID = "c"))
...
#Generates the random sample. Remove this row when you're ready to run the loop for every county.
#sample <- sample.int(nrow(counties), 10, replace = FALSE)

#Creates the dataframe that the road data will be placed into. For now, it is designed with the intent of creating a random sample because trying to do it for the entire nation would take too long. I've tried. FIPSCode is there to make filtering the territories out of the data easier. We could keep it in the dataframe for this exact purpose. Remove the [sample[1]] from each reference to the counties dataset when you're ready to run the loop for every county.
Compilation <- data.frame(StateID = counties$STATEFP, CountyID = counties$COUNTYFP, No = 1, State = counties$STATE_NAME, County = counties$NAME)

#This loop adds the random sample to the dataframe. Remove this entire loop when you're ready to run the loop for every county.
...
##may need to rename certain columns so everything matches for joins


## makes a key for each county to be identified
counties$GEOID <- paste0(counties$STATEFP, counties$COUNTYFP)
dat2$GEOID <- paste0(dat2$STATEFP, dat2$COUNTYFP)
...
##may need to re cast county id column in dat1. countyfp column in counties df are characters while county id are doubles 
